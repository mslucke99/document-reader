...
class Food_Item:
    ...
    def get_titles(self):
        ...
        if (self.ingredients != None):
            ...
            self.numbered_values[str(i)] = self.ingredients
            i+=1
            
        if (self.nutrition_data != None):
            str_val += str(i) + ". Nutrition Data"  + "\n"
            self.numbered_values[str(i)] = self.nutrition_data.repr()
            i+=1

        if (self.allergies != None):
            str_val += str(i) + ". Possible Allergies"  + "\n"
            self.numbered_values[str(i)] = self.allergies
            i+=1

        if (self.conservation != None):
            str_val += str(i)+ ". Method of Conservation"  + "\n"
            self.numbered_values[str(i)] = self.conservation
            i+=1

        if (self.quantity != None):
            str_val += str(i) + ". Quantity"  + "\n"
            self.numbered_values[str(i)] = self.quantity
            i+=1

        str_val += str(i) + ". Exit" + "\n"
        self.exit_num = str(i)
        self.numbered_values[str(i)] = None
        return str_val
